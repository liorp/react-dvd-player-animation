import React, { useState, useRef, PropsWithChildren, Fragment } from "react";
import useAnimationFrame from "./useAnimationFrame";
import { DVDLogoAnimationProps } from "./types";

const WIDTH_DVD_LOGO = 200;
const HEIGHT_DVD_LOGO = 97.45;
...
export const DVDLogoAnimation = ({
  height = 1000,
  width = 1000,
  logoHeight = HEIGHT_DVD_LOGO,
  logoWidth = WIDTH_DVD_LOGO,
  xSpeed = 2,
  ySpeed = 2,
  children,
}: PropsWithChildren<DVDLogoAnimationProps>) => {
  const [x, setX] = useState(getRandomNumber(0, width - WIDTH_DVD_LOGO));
  const [y, setY] = useState(getRandomNumber(0, height - HEIGHT_DVD_LOGO));
  const _xSpeed = useRef(xSpeed);
  const _ySpeed = useRef(ySpeed);
  const [h, setH] = useState(getRandomNumber(0, 360));
  const [s, setS] = useState(getRandomNumber(0, 101));
  const [l, setL] = useState(getRandomNumber(0, 101));

  const setRandomColors = () => {
    setH(() => getRandomNumber(0, 360));
    setS(() => getRandomNumber(45, 101));
    setL(() => getRandomNumber(0, 65));
  };

  const moveDVDLogo = () => {
    setX((v) => {
      if (v + logoWidth === width || v === 0) {
        _xSpeed.current = -_xSpeed.current;
        setRandomColors();
      }
      return clamp(v + _xSpeed.current, 0, width - logoWidth);
    });
    setY((v) => {
      if (v + logoHeight === height || v === 0) {
        _ySpeed.current = -_ySpeed.current;
        setRandomColors();
      }
      return clamp(v + _ySpeed.current, 0, height - logoHeight);
    });
  };

  useAnimationFrame(moveDVDLogo);

  return (
    <svg height="100%" width="100%">
      <g>
        <g fill={`hsl(${h}, ${s}%, ${l}%)`} transform={`translate(${x}, ${y})`}>
          {children ?? (
            <>
